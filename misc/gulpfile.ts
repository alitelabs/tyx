...
export class Gulpfile {
  ...
  public compileSamples(): any {
    const folders = this.getFolders('./samples');

    const tasks = folders.map(function (sampleFolder) {
      const buildDestination = './build/samples/' + sampleFolder + '/';
  ...
  public installModules(): any {
    const folders = this.getFolders('./build/samples');

    const tasks = folders.map(function (sampleFolder) {
      const packageJsonSourcePath = './samples/' + sampleFolder + '/package.json';
      const installPath = './build/samples/' + sampleFolder;
  ...
  public copyServerlessFiles(): any {
    ...
    const tasks = folders.map(function (sampleFolder) {
      const serverlessYmlSourcePath = './samples/' + sampleFolder + '/serverless.yml';
      const copyToPath = './build/samples/' + sampleFolder;
  ...
  public copyPublicFiles(): any {
    ...
    const tasks = folders.map(function (sampleFolder) {
      const publicSourcePath = './samples/' + sampleFolder + '/public/**/*';
      const copyToPath = './build/samples/' + sampleFolder + '/public';
