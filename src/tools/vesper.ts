import { GraphQLToolkit } from './graphql';

const ENTITY = '';
const GET = '';
const SEARCH = '';

export default function codegen(schema: GraphQLToolkit | any, folder: string, extension?: string) {
  const ext = extension || 'gql';
  const fs = require('fs');
  fs.writeFileSync(
    `${folder}/schema/import.${ext}`,
    GraphQLToolkit.prolog().replace('scalar Date', '#scalar Date')
  );
